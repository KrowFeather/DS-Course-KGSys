...
前置知识：[计数排序](counting-sort.md)，[基数排序](radix-sort.md)。
...
有了这个定理，求两子串最长公共前缀就转化为了 [RMQ 问题](rmq.md)。
