...
前置知识：[随机函数](random.md) 和 [概率初步](basic-conception.md)
...
详见 [随机增量法](random-incremental.md)。
...
-   保证算法过程中进行的「操作」具有（某种意义上的）均匀性。例如 [模拟退火](simulated-annealing.md) 算法。
...
详见 [概率不等式](concentration-inequality.md) 页面。
