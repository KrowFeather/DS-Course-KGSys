...
类比一维的情形，$S_{i,j}$ 应该可以基于 $S_{i-1,j}$ 或 $S_{i,j-1}$ 计算，从而避免重复计算前面若干项的和。但是，如果直接将 $S_{i-1,j}$ 和 $S_{i,j-1}$ 相加，再加上 $A_{i,j}$，会导致重复计算 $S_{i-1,j-1}$ 这一重叠部分的前缀和，所以还需要再将这部分减掉。这就是 [容斥原理](inclusion-exclusion-principle.md)。由此得到如下递推关系：
...
子集和的逆操作需要通过 [容斥原理](inclusion-exclusion-principle.md) 进行。子集和问题也是快速莫比乌斯变换的必要步骤之一。
...
树上差分通常会结合 [树基础](tree-basic.md) 和 [最近公共祖先](lca.md) 来进行考察。树上差分又分为 **点差分** 与 **边差分**，在实现上会稍有不同。
...
![](../docs/basic/images/prefix_sum1.png)
...
![](../docs/basic/images/prefix_sum2.png)
