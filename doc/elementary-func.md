...
参见 [Newton's Method](newton2.md#newtons-method).
...
=== "多项式对数函数"
    首先，对于多项式 $f(x)$，若 $\ln{f(x)}$ 存在，则由其 [定义](intro2.md#复合)，其必须满足：
...
使用 [Newton's Method](newton2.md#newtons-method) 即可在 $O(n\log{n})$ 的时间复杂度内解决多项式 $\exp$。
