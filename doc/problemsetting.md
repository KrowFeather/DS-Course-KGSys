...
-   [LaTeX 入门](../docs/tools/latex.md#图表)
...
使用时请注意 [LaTeX 公式的格式要求](../docs/intro/format.md)。
...
另外，出题时推荐使用 [testlib](../docs/tools/testlib/generator.md) 来造数据，可以保证在不同平台上同一个种子生成的随机数相同，并且种子会依据命令行参数自动生成。
...
真正均匀随机的方法是利用 [Prüfer 序列](prufer.md)，先生成一个随机 Prüfer 序列，再通过序列生成树。这样做的话，树的期望高度为 $O(\sqrt n)$。
...
[SPJ 编写教程](../docs/tools/special-judge.md)
...
Polygon 是一个功能非常强大的多人合作出题平台，可以作为在任何网站（使用 package 功能导出到不支持 Polygon 的网站）多人合作出题的首选方案，单人出题（尤其是在不同设备上出题）时也是很不错的选择，使用方法参见 [Polygon 简介](../docs/tools/polygon.md)。
...
参考 [Polygon 简介](../docs/tools/polygon.md)。
