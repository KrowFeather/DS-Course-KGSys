...
在对一张 $n$ 个点 $m$ 条边的图进行 [DFS](dfs.md) 时，由于每个节点和每条边都只会被访问常数次，复杂度为 $\Theta(n+m)$。
...
本文主要从算法分析的角度对复杂度进行了介绍，如果有兴趣的话可以在 [计算复杂性](cc-basic.md) 进行更深入的了解。
