...
如果序列 $f$ 满足积性（是 [积性函数](basic5.md#积性函数)）：$\forall i\perp j, \; f_{ij} = f_i f_j$，那么其 DGF 可以由质数幂处的取值表示：
...
以 [洛谷 P3768 简单的数学题](du.md#问题二) 为例，我们要对 $f_i = i^2\varphi(i)$ 构造一个满足上述条件的积性序列 $g$。由于 $f$ 是积性的，考虑其 DGF
