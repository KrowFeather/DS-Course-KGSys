...
-   数据结构中，[线段树](seg.md) 等结构可以维护幺半群的信息，而很多 DP 问题的递推关系可以抽象成这样的幺半群结构；
-   组合数学中，[Pólya 计数原理](polya.md) 的严格表述和证明需要用到群论的相关概念。
...
???+ example "群的例子"
    ...
    -   空间对称群（symmetry group）：对于给定几何图形，能够使其与自身重合的变换全体也在映射的复合下构成群。这描述了该几何图形的空间对称性。具体例子可以参考 [常见空间对称群](polya.md#常见空间对称群)。
    -   整数的加法群：整数集 $\mathbf Z$ 在加法 $+$ 运算下构成群 $(\mathbf Z,+)$。单位元是 $0$，逆元是相反数。
    -   整数模 $n$ 乘法群（multiplicative group of integers modulo $n$）：给定模 $n$，所有与 $n$ 互质的整数对应的 [同余类](basic5.md#同余类与剩余系)，在乘法运算下构成群 $((\mathbf Z/n\mathbf Z)^\times,\times)$。单位元是 $\bar 1$，逆元就是模 $n$ 的 [乘法逆元](inverse3.md)（对应的同余类），其存在性由 [裴蜀定理](bezouts.md) 保证。
...
???+ note "思路分析"
    这道题是动态 DP 模板，一种复杂度正确的代码实现需要用到 [全局平衡二叉树](global-bst.md)，具体样例代码也在对应页面。这里仅仅结合该题情景，分析建模的过程。
