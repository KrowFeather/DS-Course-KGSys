...
所以 `constexpr` 可以用来替换宏定义的常量，规避 [宏定义的风险](basic2.md#define-命令)。

算法题中可以使用 `constexpr` 存储数据规模较小的变量，以消除对应的运行时计算开销。尤为常见在「[打表](dictionary.md)」技巧中，使用 `constexpr` 修饰的数组等容器存储答案。
